...
// //   useEffect(() => {
// //     (async () => {
// //       const data = await getAllCalendarEvents(lang);
// //       if (data) {
// //         setEvents(
// //           data.map((r) => ({
// //             ...r,
...
//   created_at?: string;
// };
// type CountRow = { legend_type?: string; count: number };
...
//       setLoading(true);
//       try {
//         const [ev, lg, cnt] = await Promise.all([
//           getAllCalendarDates(lang),
//           getAllCalendarLegend(lang),
//           getCalendarEventsCount(lang),
//         ]);

//         const mapDates = (rows: CalendarType[]) =>
//           (rows ?? []).map((r) => ({
//             ...r,
//             gregorian_date: new Date(r.gregorian_date),
//             created_at: new Date(r.created_at),
...
//         setEvents(mapDates(ev ?? []));
//         setLegend(lg ?? []);

//         const cMap: Record<string, number> = {};
//         (cnt ?? []).forEach((row: CountRow) => {
//           const key = row.legend_type ?? "";
//           if (key) cMap[key] = row.count;
//         });
//         setCounts(cMap);
//       } catch (e) {
//         console.error(e);
...
//   // ─── legend items (today/upcoming + all legend types) ───
//   function Legend() {
//     const items = (legend ?? [])
//       .map((l) => ({
//         key: l.legend_type ?? "",
//         count: counts[l.legend_type ?? ""] ?? 0,
//       }))
//       .filter((i) => i.key);
...
//             <ThemedText>{loading ? t("loading") : t("noResults")}</ThemedText>
//           </View>
//         }
//       />
//     </ThemedView>
...
//   },
//   legendPillText: { fontSize: 12, color: "#374151" },
//   description: { fontSize: 14, color: "#6b7280", lineHeight: 20 },
//   datesContainer: { gap: 4 },
//   dateRow: { flexDirection: "row", alignItems: "center", gap: 8 },
//   countdownRow: { flexDirection: "row", alignItems: "center", gap: 8 },
...
import {
  getAllCalendarDates,
  getAllCalendarLegend,      // ⬅️ we’ll call this with lang
  getCalendarEventsCount,    // optional: show counts next to legend types
} from "@/db/queries/calendar";
...
const RenderCalendar: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const [ev, lg, cnt] = await Promise.all([
          getAllCalendarDates(lang),
          getAllCalendarLegend(lang),     // ⬅️ legend pulled for lang
          getCalendarEventsCount(lang),   // ⬅️ counts per legend_type (lang)
        ]);
  ...
  // ─── legend (uses getAllCalendarLegend(lang) data) ───
  function Legend() {
    const items = (legend ?? [])
      .map((l) => ({
        key: l.legend_type ?? "",
    ...
    return (
      <View style={styles.legend}>
        ...
        <View style={styles.legendGrid}>
          ...
          {/* dynamic legend types from DB for this language */}
          {items.map(({ key, count }) => (
            <LegendIcon
              key={key}
              icon="pricetag"
        ...
        <View style={styles.cardContent}>
          <View style={styles.cardMain}>
            ...
            <View style={styles.datesContainer}>
              ...
              <DateRow
                icon="location"
                text={event.gregorian_date.toLocaleDateString(language ?? "en", {
                  weekday: "long",
                  year: "numeric",
                  month: "long",
                  day: "numeric",
  ...
  return (
    <ThemedView style={styles.container}>
      {/* Header */}
      <View style={styles.header}>
        <View className="row" style={styles.headerTitle}>
          <Ionicons name="moon" size={24} color="#059669" />
          <ThemedText style={styles.title}>{t("calendarTitle")}</ThemedText>
        </View>
        <ThemedText style={styles.subtitle}>
      ...
      {/* Legend ABOVE the list (uses getAllCalendarLegend(lang)) */}
      <Legend />
