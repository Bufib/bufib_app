...
export const PodcastPlayer: React.FC<PodcastPlayerProps> = ({ podcast }) => {
  ...
  useEffect(() => {
    if (
      ...
      try {
        ...
        const errorMsg = err instanceof Error ? err.message : "Unknown playback error";
        console.error("Synchronous Auto-play failed:", errorMsg);
        setPlayerError(`Failed to start playback: ${errorMsg}`);
      }
    }
  ...
  const handleStream = useCallback(() => {
    if (!podcast.sound_path) { setPlayerError("Audio path missing."); return; }
    setIsPreparingStream(true);
    setPlayerError(null);
    setSourceUri(null);
    setDownloadProgress(0);
  ...
  const goBack = useCallback(() => {
    ...
    const newPosition = Math.max(0, status.currentTime - 15000);
    player.seekTo(newPosition);
  }, [player, status?.isLoaded, status?.currentTime, status?.duration, playerError]);
  ...
  // Updated Format time helper with logging
  const formatTime = (label: string, millis: number | undefined | null): string => {
    // Log the input value received by the function
    // console.log(`formatTime (${label}) received millis:`, millis);
    if (millis === undefined || millis === null || isNaN(millis) || millis < 0) {
      // console.log(`formatTime (${label}) returning 0:00 due to invalid input.`);
      return "0:00";
    }
    try {
      ...
      // console.log(`formatTime (${label}) calculated result:`, result); // Log the output
      return result;
    } catch (e) {
      // Log any unexpected errors during formatting
      console.error(`formatTime (${label}) error:`, e);
      return "0:00"; // Return default value on error
    }
  };
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>{podcast.title}</Text>
      <Text style={styles.desc} numberOfLines={2}>{podcast.description}</Text>

      {playerError && <Text style={styles.errorText}>Error: {playerError}</Text>}
      {isLoading && !playerError && <ActivityIndicator style={styles.loader} />}
      ...
      {showInitialButtons && (
        <>
          <Button title="Stream Episode" onPress={handleStream} disabled={isLoading} />
          <View style={styles.spacer} />
          <Button title="Download & Play" onPress={handleDownloadAndPlay} disabled={isLoading} />
        </>
      )}
      ...
      {showPlaybackControls && !playerError && (
        <>
          <View style={styles.timeContainer}>
            {/* Pass labels to formatTime for clearer debugging */}
            <Text>Current: {formatTime('currentTime', status?.currentTime)}</Text>
            <Text>Duration: {formatTime('duration', status?.duration)}</Text>
          </View>
          {/* Optional: <Slider /> component here */}
          <View style={styles.controls}>
            <Button title="⏪ 15s" onPress={goBack} disabled={controlsDisabled}/>
            <Button title={status?.playing ? "Pause" : "Play"} onPress={togglePlayPause} disabled={controlsDisabled}/>
            <Button title="15s ⏩" onPress={goForward} disabled={controlsDisabled}/>
          </View>
          <Button title="Stop" onPress={stopPlayback} disabled={controlsDisabled} />
        </>
      )}
    </View>
  );
...
// --- Styles ---
const styles = StyleSheet.create({
    container: { padding: 16, backgroundColor: "#fff", borderRadius: 8, marginVertical: 8, shadowColor: "#000", shadowOpacity: 0.1, shadowRadius: 4, elevation: 2 },
    title: { fontSize: 18, fontWeight: "600" },
    desc: { fontSize: 14, color: "#666", marginVertical: 8 },
    loader: { marginVertical: 16, alignSelf: 'center' },
    spacer: { height: 8 },
    errorText: { color: 'red', marginVertical: 8, textAlign: 'center' },
    progressContainer: { width: "100%", height: 6, backgroundColor: "#e0e0e0", borderRadius: 3, overflow: "hidden", marginVertical: 8 },
    progressBar: { height: "100%", backgroundColor: "#3b82f6" },
    progressText: { textAlign: "center", fontSize: 12, color: '#444', marginBottom: 8 },
    timeContainer: { flexDirection: 'row', justifyContent: 'space-between', marginVertical: 5, paddingHorizontal: 10 },
    controls: { flexDirection: "row", justifyContent: "space-around", alignItems: 'center', marginVertical: 12 },
});
