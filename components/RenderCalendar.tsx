...
// // import CalendarLegendRow from "./CalendarLegendRow";
...
// //       try {
// //         setLoading(true);
// //         const data = await getAllCalendarDates(lang);
...
//   // Build the exact same color mapping the legend uses (same names, same order)
...
//             <ThemedText style={styles.badgeText}>{item.legend_type}</ThemedText>
//           </View>
//         </View>
...
//         {/* Description (always shown when present) */}
//         {!!item.description && (
...
//   meta: { marginTop: 10, fontSize: 12, fontStyle: "italic", opacity: 0.7 },
...
const RenderCalendar: React.FC = () => {
  ...
  useEffect(() => {
    const hasToday = events.some((e) => dayDiffFromToday(e.gregorian_date) === 0);
    if (hasToday) {
