  ...
  const schema = useMemo(
    () =>
      z
        .object({
          username: z
            .string({ required_error: t("signUpUsernameNotEmpty") })
            .min(3, t("signUpUserNameMin"))
            .regex(/^[a-zA-Z0-9_-]+$/, t("signUpUsernameInvalidChars")),
          email: z
            .string({ required_error: t("signUpEmailNotEmpty") })
            .email(t("signUpEmailInvalid")),
          password: z
            .string({ required_error: t("signUpPasswordNotEmpty") })
            .min(8, t("signUpUserPasswordMin"))
            .regex(
              /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+=\-\[\]{};\:"\\|'<>?,./~]).{8,}$/,
              t("signUpUserPasswordFormat")
            ),
          confirmPassword: z
            .string({ required_error: t("signUpPasswordNotEmpty") })
            .min(8, t("signUpUserPasswordMin")),
