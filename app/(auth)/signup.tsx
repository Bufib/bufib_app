...
export default function SignUpScreen() {
  ...
  const onCaptchaMessage = async (event: any) => {
    ...
    // 1) Wenn das Widget gerade geöffnet wurde
    if (data === "open") {
      // Optional: du könntest hier loggen, dass’s geöffnet ist.
      return;
    }

    // 2) Wenn wir einen gültigen Token haben
    if (event.success) {
      // Speichere/verwende den Token
      const token = data;
      captchaRef.current?.hide();
      event.markUsed();

      // Rufe Supabase.signUp() mit den zwischengespeicherten formData
    ...
    // 3) Wenn die Challenge abgelaufen ist
    if (data === "challenge-expired") {
      event.reset(); // Neu laden, damit User erneut versuchen kann
      return;
    }

    // 4) Sonstige Fehler (inkl. "cancel")
    captchaRef.current?.hide();
    setTimeout(() => {
      // Kleines Delay, damit hide() vorher ausgeführt wird
      Alert.alert("Fehler", cancleCaptcha);
    }, 100);
  };

  /** onSubmit Handler: nur Formular-Validierung → hCaptcha starten */
  const onSubmit = async (formData: SignUpFormValues) => {
    handleSignup(formData);
  };
